...
def assess(iid: int, latest: dict[int, dict], hourly: dict[int, dict],
           ts_fn: Callable[..., list[dict]], *, deep: bool = False, long: bool = False) -> dict[str, Any]:
    """Single-item 'why': live price vs recent baselines + volume z + slope + phase. `deep` adds the
    full multi-timeframe table (for `why`); `long` adds just the 3-month baseline (for the slow-pump
    check, one extra fetch); the lean path (one 1h fetch) feeds `go`."""
    lp, hp = latest.get(iid) or {}, hourly.get(iid) or {}
    high, low = lp.get("high"), lp.get("low")
    live_mid = (high + low) / 2 if (high and low) else None
    vol_binding = min(hp.get("highPriceVolume") or 0, hp.get("lowPriceVolume") or 0)
    res: dict[str, Any] = {"live_bid": low, "live_ask": high, "live_mid": live_mid,
                           ...
                           # structurally corner-able: thin AND valuable → a few offers move the price
                           "manipulable": vol_binding <= config.MANIP_VOL_MAX
                           and (live_mid or 0) >= config.MANIP_PRICE_MIN}
    if deep:
        steps = [("5m", "1d"), ("1h", "2wk"), ("6h", "3mo"), ("24h", "30d")]
    elif long:
        steps = [("1h", "2wk"), ("6h", "3mo")]
    else:
        steps = [("1h", "2wk")]
...
def is_buyable(a: dict[str, Any]) -> bool:
    ...
    if abs(div) < config.ANOMALY_DIV_MIN:
        # normal vs the 2-week median — but on a MANIPULABLE (thin + valuable) item, also check the
        # LONGER baseline: a slow pump can drift the 2wk median up with it, hiding in plain sight,
        # while it still sits well above its 3-month/30-day level. Elevated there → don't chase.
        if a.get("manipulable"):
            for label in ("3mo", "30d"):
                base = a.get("baselines", {}).get(label)
                if base and (live - base) / base > config.ANOMALY_DIV_MIN:
                    return False
        # else "normal"-priced UNLESS it's a steep active decline — a falling knife heading for the
...
def summary_line(a: dict[str, Any]) -> str:
    ...
    if abs(div) < config.ANOMALY_DIV_MIN:
        if a.get("manipulable"):  # slow pump hides near the 2wk median but sits high vs the 3mo/30d
            for label in ("3mo", "30d"):
                base = a.get("baselines", {}).get(label)
                if base and (a["live_mid"] - base) / base > config.ANOMALY_DIV_MIN:
                    return (f"⚠ thin/manipulable: near {lbl} norm but +{(a['live_mid'] - base) / base * 100:.0f}% "
                            f"vs {label} {base:,.0f} — likely slow pump, don't chase")
        ...
        note = " · ⚠ thin/manipulable, size down" if a.get("manipulable") else ""
        return f"price normal (~{ref:,.0f} {lbl} norm){note}"
